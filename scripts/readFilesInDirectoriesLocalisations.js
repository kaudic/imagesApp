...
const getFiles = async (source, localisation) => {
    ...
    readdir(source, { withFileTypes: true }, async (err, files) => {
        if (err) {
                    ...
                    try {
                        ...
                        if (fingerPrints === undefined) {
                            ...
                            if (isBinary && isBinary.fingerPrints != undefined) {
                                ...
                                imageSource.push({ fileName: img, localisation: localisation, fingerPrints: fingerPrints });
                            }
            ...
            dirNames.forEach((dirName) => {
                getFiles(`${source}/${dirName}`, localisation);
            });

            // write in data repo
            const repoPath = path.normalize(`${__dirname}/../data/localisation${localisation}.json`);
...
const source = path.normalize(`${__dirname}/../../../../pictures/2013/nov/Andalousie`);
// ! change year here
getFiles(source, 'Andalousie');
