...
const getFiles = async (source, event) => {
    ...
    readdir(source, { withFileTypes: true }, async (err, files) => {
        if (err) {
                    ...
                    try {
                        ...
                        if (fingerPrints === undefined) {
                            ...
                            if (isBinary && isBinary.fingerPrints != undefined) {
                                ...
                                imageSource.push({ fileName: img, event: event, fingerPrints: fingerPrints });
                            }
            ...
            dirNames.forEach((dirName) => {
                getFiles(`${source}/${dirName}`, event);
            });

            // write in data repo
            const repoPath = path.normalize(`${__dirname}/../data/event${event}.json`);
