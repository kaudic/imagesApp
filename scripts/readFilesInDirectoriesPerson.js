...
const getFiles = async (source, person) => {
    ...
    readdir(source, { withFileTypes: true }, async (err, files) => {
        if (err) {
                    ...
                    try {
                        ...
                        if (fingerPrints === undefined) {
                            ...
                            if (isBinary && isBinary.fingerPrints != undefined) {
                                ...
                                imageSource.push({ fileName: img, person: person, fingerPrints: fingerPrints });
                            }
            ...
            dirNames.forEach((dirName) => {
                getFiles(`${source}/${dirName}`, person);
            });

            // write in data repo
            const repoPath = path.normalize(`${__dirname}/../data/person${person}2.json`);
...
const source = path.normalize(`${__dirname}/../../../../pictures/2019/Malo`);
// ! change year here
getFiles(source, 'Malo');
